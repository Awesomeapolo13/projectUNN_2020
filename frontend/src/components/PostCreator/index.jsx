...
import {Container, Form, Button, Spinner, Jumbotron} from 'react-bootstrap';
    ...
    return (
        <>
            <Container>
                {props.postCreated ?
                    <Jumbotron>
                        <p>{props.changePostMessage.message}</p>
                        <Button
                            variant={'primary'}
                            type={'button'}
                            onClick={() => props.actions.createPostAgain()}
                        >Сделать новую публикацию</Button>
                    </Jumbotron>
                    :
                    <Form>
                        <Form.Group controlId={'newPostTitle'}>
                            ...
                            />
                            {props.isPostTitleInvalid && <Form.Control.Feedback type={'invalid'}>Введитие заголовок
                                публикации</Form.Control.Feedback>}
                        </Form.Group>
                        ...
                        <Form.Group controlId={'newPostText'}>
                            ...
                            />
                            {props.isPostTextInvalid &&
                            <Form.Control.Feedback type={'invalid'}>Введитие текст публикации</Form.Control.Feedback>}
                        </Form.Group>
                        <Button
                            variant={'primary'}
                            type={'button'}
                            ...
                            onClick={() => {
                                props.actions.createPost(props.postTitle, props.postType, props.postText)
                            }}
                        >Опубликовать</Button>
                    </Form>
                }
                {props.isPostCreating && <Spinner animation={'border'} role={'status'} variant={'dark'}/>}
                {props.isPostCreating && <Spinner animation={'border'} role={'status'} variant={'dark'}/>}
                {props.isPostCreating && <Spinner animation={'border'} role={'status'} variant={'dark'}/>}
            </Container>
        </>
